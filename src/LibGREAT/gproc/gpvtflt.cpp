...
int great::t_gpvtflt::_processEpoch(const t_gtime& runEpoch)
{
    ...
    if (_ambfix && _observ != OBSCOMBIN::IONO_FREE)
    {
        _ambfix->setELE(_crt_ele);
        _ambfix->setAZI(_crt_azi);
...
void great::t_gpvtflt::_prtOut(t_gtime& epoch, t_gallpar& X, const SymmetricMatrix& Q, vector<t_gsatdata>& data, ostringstream& os, xml_node& node, bool saveProd)
{
    ...
    _last_flt.sow = epoch.sow() + epoch.dsec();
    _last_flt.x = xyz_ecc[0];
    _last_flt.y = xyz_ecc[1];
    _last_flt.z = xyz_ecc[2];
    _last_flt.vx = vRec[0];
    _last_flt.vy = vRec[1];
    _last_flt.vz = vRec[2];
    _last_flt.xrms = Xrms;
    _last_flt.yrms = Yrms;
    _last_flt.zrms = Zrms;
    _last_flt.vxrms = Vxrms;
    _last_flt.vyrms = Vyrms;
    _last_flt.vzrms = Vzrms;
    _last_flt.nsat = nsat;
    _last_flt.pdop = pdop;
    _last_flt.sigma0 = _sig_unit;
