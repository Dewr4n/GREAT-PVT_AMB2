...
namespace great
{
    ...
    int t_gambiguity::processBatch(const t_gtime& t, t_gflt* gflt, string mode)
    {
        ...
        for (unsigned int i = 0; i < param_float.parNumber(); ++i)
        {
            ...
            if (par.parType == par_type::SION && par.site == _site)
            {
                ...
                _sion_each.push_back(value);
                sum_sion += value;
            }
        }
        _sion_avg = _sion_each.empty() ? 0.0 : sum_sion / _sion_each.size();
    ...
    void t_gambiguity::_initFeaturefile()
    {
        ...
        if (_featurefile)
        {
            ...
            os << "Nfixed_shk,Nfixed_shk2,Nfixed_dec_shk,Nfixed_dec_shk2,elev_each,avg_elev,snr_each,avg_snr,SION_each,avg_SION,ZTD" << endl;
    ...
    void t_gambiguity::_writeFeature(const t_flt_rec& rec)
    {
        ...
        os << rec.sow << ','
            ...
            << joinD(_sion_each) << ',' << _sion_avg << ',' << _ZTD << '\n';
