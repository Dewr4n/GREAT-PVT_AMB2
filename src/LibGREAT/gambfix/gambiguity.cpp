...
namespace great
{
    ...
    int t_gambiguity::processBatch(const t_gtime& t, t_gflt* gflt, string mode)
    {
        ...
        // float solution parameters and update vector prior to constraint
        t_gallpar param_float = fltpreAMB.param();
        ColumnVector dx_float = fltpreAMB.dx();
        ...
        if (idxC >= 0 && idxC < dx_float.Nrows())
            crd_float[0] += dx_float(idxC + 1);
        if (idyC >= 0 && idyC < dx_float.Nrows())
            crd_float[1] += dx_float(idyC + 1);
        if (idzC >= 0 && idzC < dx_float.Nrows())
            crd_float[2] += dx_float(idzC + 1);
        ...
        // store float-stage slant ionosphere parameters
        _sion_all.clear();
        _sion_ref_orig.clear();
        _sion_tgt_orig.clear();

        map<string, double> sion_map;
        for (unsigned int i = 0; i < param_float.parNumber(); ++i)
        {
            const t_gpar& par = param_float[i];
            if (par.parType == par_type::SION && par.site == _site)
            {
                double value = par.value();
                const int idx = par.index;
                if (idx > 0 && idx <= dx_float.Nrows())
                {
                    value += dx_float(idx);
                }
                else if (i < static_cast<unsigned int>(dx_float.Nrows()))
                {
                    value += dx_float(i + 1);
                }
                sion_map[par.prn] = value;
            }
        }

        _sion_all.assign(_sat_all.size(), 0.0);
        for (size_t i = 0; i < _sat_all.size(); ++i)
        {
            auto it = sion_map.find(_sat_all[i]);
            if (it != sion_map.end())
            {
                _sion_all[i] = it->second;
            }
        }

        _fillAligned(_sat_ref_orig, _sion_all, _sion_ref_orig);
        _fillAligned(_sat_tgt_orig, _sion_all, _sion_tgt_orig);

        // float-stage zenith tropospheric delay
        _ZTD = 0.0;
        int idx_trp_float = param_float.getParam(_site, par_type::TRP, "");
        if (idx_trp_float >= 0)
        {
            const t_gpar& par = param_float.getPar(idx_trp_float);
            double value = par.value();
            const int idx = par.index;
            if (idx > 0 && idx <= dx_float.Nrows())
            {
            ...
            else if (idx_trp_float < dx_float.Nrows())
            {
                value += dx_float(idx_trp_float + 1);
            }
            _ZTD = par.zhd + value;
        }
    ...
    bool t_gambiguity::_ambSolve(t_gamb_cmn* amb_cmn, vector<int>& fixed_amb, string mode)
    {
        ...
        _numsd_dec_pre = 0;
        _sat_ref_dec_pre.clear();
        _sat_tgt_dec_pre.clear();
        _Nfloat_dec_pre.clear();
        _Q_dec_pre.clear();
        _Z_dec_pre.clear();
        ...
        if (_DD.size() < _full_fix_num)
        {
            ...
            _numsd_orig = static_cast<int>(_sat_ref_orig.size());
            _Nfloat_orig.assign(_numsd_orig, 0.0);
        ...
        while ((ratio < _ratio || (boot < _boot && mode == "NL")) && _part_fix)
        {
            ...
            {
                // Even though PAR cannot continue, record the shrunken set so that
                // feature logging reflects the current epoch instead of reusing
                // the previous successful fix.
                _numsd_shk = static_cast<int>(value.size());
                _sat_ref_shk.clear();
                _sat_tgt_shk.clear();
                for (const auto& dd : _DD)
                {
                ...
                // keep decorrelated diagnostics in sync with the shrunken set
                vector<int> dummy_fixed;
                double dummy_boot = 0.0;
                double dummy_ratio = _lambdaSearch(covariance, value, dummy_fixed, &dummy_boot);
                if (dummy_ratio > 99.0)
                    dummy_ratio = 99.0;
    ...
    void t_gambiguity::_initFeaturefile()
    {
        ...
        if (_featurefile)
        {
            ...
            os << "Nfixed_shk,Nfixed_shk2,Nfixed_dec_shk,Nfixed_dec_shk2,sat_all,elev_all,snr_all,SION_all,";
            os << "elev_ref_orig,snr_ref_orig,SION_ref_orig,elev_tgt_orig,snr_tgt_orig,SION_tgt_orig,ZTD" << endl;
    ...
    void t_gambiguity::_writeFeature(const t_flt_rec& rec)
    {
        ...
        // Refresh the slant ionosphere vectors so they reflect the
        // latest reference/target satellite ordering recorded in the
        // CSV output.  The DD set can be shrunk after the float-stage
        // environment features are collected, so recomputing here keeps
        // `SION_ref_orig`/`SION_tgt_orig` aligned with the satellite
        // lists we are about to serialize.
        _fillAligned(_sat_ref_orig, _sion_all, _sion_ref_orig);
        _fillAligned(_sat_tgt_orig, _sion_all, _sion_tgt_orig);
        ...
        os << rec.sow << ','
            ...
            << joinS(_sat_all) << ',' << joinD(_elev_all) << ',' << joinD(_snr_all) << ',' << joinD(_sion_all) << ','
            << joinD(_elev_ref_orig) << ',' << joinD(_snr_ref_orig) << ',' << joinD(_sion_ref_orig) << ','
            << joinD(_elev_tgt_orig) << ',' << joinD(_snr_tgt_orig) << ',' << joinD(_sion_tgt_orig) << ','
            << _ZTD << '\n';
    ...
    void t_gambiguity::_refreshEnvFeatures()
    {
        _sat_all.clear();
        _sat_all_index.clear();
        _elev_all.clear();
        _snr_all.clear();
        _elev_ref_orig.clear();
        _snr_ref_orig.clear();
        _elev_tgt_orig.clear();
        _snr_tgt_orig.clear();

        auto get_snr = [&](const string& sat) {
        ...
        _sat_all.reserve(_ELE.size());
        _elev_all.reserve(_ELE.size());
        _snr_all.reserve(_ELE.size());
        for (const auto& kv : _ELE)
        {
            const string& sat = kv.first;
            _sat_all_index[sat] = _sat_all.size();
            _sat_all.push_back(sat);
            _elev_all.push_back(kv.second);
            _snr_all.push_back(get_snr(sat));
        }

        _fillAligned(_sat_ref_orig, _elev_all, _elev_ref_orig);
        _fillAligned(_sat_ref_orig, _snr_all, _snr_ref_orig);
        _fillAligned(_sat_tgt_orig, _elev_all, _elev_tgt_orig);
        _fillAligned(_sat_tgt_orig, _snr_all, _snr_tgt_orig);
    }

    void t_gambiguity::_fillAligned(const vector<string>& sat_order,
        const vector<double>& source,
        vector<double>& dest) const
    {
        dest.assign(sat_order.size(), 0.0);
        for (size_t i = 0; i < sat_order.size(); ++i)
        {
            auto it = _sat_all_index.find(sat_order[i]);
            if (it != _sat_all_index.end())
            {
