    ...
    */
    ...
    public:
        ...
        */
        ...
        void setAZI(const map<string, double>& crt_azi) { _AZI = crt_azi; }
        ...
        */
        ...
        /** @brief log .flt record for fixed epoch */
        void logFlt(const t_flt_rec& rec);
    ...
    protected:
        ...
        t_giof* _featurefile = nullptr; ///< feature output file
        t_giof* _wliffile = nullptr; ///< IF/WL ambiguity file
        ...
        int _full_fix_num;                                        ///< full fix number
                ...
                // storage for intermediate feature output
        std::vector<double> _az;   ///< decorrelated ambiguity vector (post-shrink)
        std::vector<double> _Zt;   ///< Z^T transformation matrix (post-shrink)
        std::vector<double> _Qz;   ///< decorrelated covariance matrix (post-shrink)
        double _s0 = 0.0;          ///< squared norm of best candidate
        double _s1 = 0.0;          ///< squared norm of second candidate
        int _numsd_dec = 0;        ///< number of SD ambiguities in decorrelated space (post-shrink)
        std::vector<std::string> _sat_ref_dec; ///< reference satellite IDs (decorrelated space)
        std::vector<std::string> _sat_tgt_dec; ///< target satellite IDs (decorrelated space)

        int _numsd_dec_pre = 0;                    ///< number of SD ambiguities in decorrelated space before shrink
        std::vector<std::string> _sat_ref_dec_pre; ///< reference satellites before shrink (decorrelated space)
        std::vector<std::string> _sat_tgt_dec_pre; ///< target satellites before shrink (decorrelated space)
        std::vector<double> _Nfloat_dec_pre;       ///< float ambiguity vector before shrink (decorrelated space)
        std::vector<double> _Q_dec_pre;            ///< covariance matrix before shrink (decorrelated space)
        std::vector<double> _Z_dec_pre;            ///< Z^T transformation before shrink (decorrelated space)

        std::vector<double> _Nfixed_dec;           ///< best integer candidate (decorrelated space)
        std::vector<double> _Nfixed_dec2;          ///< second-best integer candidate (decorrelated space)
        std::vector<double> _ibias;                ///< integer shift applied to float ambiguities

        int _numsd_orig = 0;                       ///< number of SD ambiguities in original space
        std::vector<std::string> _sat_ref_orig;    ///< reference satellite IDs
        std::vector<std::string> _sat_tgt_orig;    ///< target satellite IDs
        std::vector<double> _Nfloat_orig;          ///< float ambiguity vector (original space)
        std::vector<double> _Q_orig;               ///< covariance matrix (original space, row-major)

        int _numsd_shk = 0;                        ///< number of SD ambiguities after PAR
        std::vector<std::string> _sat_ref_shk;     ///< reference satellite IDs after PAR
        std::vector<std::string> _sat_tgt_shk;     ///< target satellite IDs after PAR
        std::vector<double> _Nfloat_shk;           ///< float ambiguity vector after PAR
        std::vector<double> _Q_shk;                ///< covariance matrix after PAR (row-major)
        std::vector<double> _Nfixed_shk;           ///< best integer candidate
        std::vector<double> _Nfixed_shk2;          ///< second-best integer candidate

        t_gtriple _xyz_float;                     ///< float position solution
        t_gtriple _xyz_fix;                       ///< fixed position solution
        t_gtriple _dxyz;                          ///< fixed minus float position correction
        t_gtriple _vel;                           ///< ECEF velocity copied from filter
        double _xrms = 0.0, _yrms = 0.0, _zrms = 0.0;    ///< position RMS from filter
        double _vxrms = 0.0, _vyrms = 0.0, _vzrms = 0.0; ///< velocity RMS from filter
        std::vector<double> _PP;                  ///< position covariance matrix (row-major 3x3)
        ColumnVector _stdx;                       ///< standard deviations from filter
        int _info = 0;                            ///< 1 if fixed, 0 otherwise
        double _pdop = 0.0;                       ///< position dilution of precision
        std::vector<std::string> _sat_all;        ///< satellite IDs observed in the current epoch
        std::unordered_map<std::string, std::size_t> _sat_all_index; ///< lookup from satellite ID to position in _sat_all
        std::vector<double> _elev_all;            ///< per-satellite elevation angles (deg)
        std::vector<double> _snr_all;             ///< per-satellite SNR (dB-Hz)
        std::vector<double> _sion_all;            ///< per-satellite slant ionosphere delays (m)
        std::vector<double> _elev_ref_orig;       ///< elevation angles of reference satellites (deg)
        std::vector<double> _snr_ref_orig;        ///< SNR of reference satellites (dB-Hz)
        std::vector<double> _sion_ref_orig;       ///< slant ionosphere of reference satellites (m)
        std::vector<double> _elev_tgt_orig;       ///< elevation angles of target satellites (deg)
        std::vector<double> _snr_tgt_orig;        ///< SNR of target satellites (dB-Hz)
        std::vector<double> _sion_tgt_orig;       ///< slant ionosphere of target satellites (m)
        double _ZTD = 0.0;                        ///< zenith tropospheric delay (m)
        double _sigma0 = 0.0;                     ///< variance factor
        double _outBoot = 0.0;                    ///< bootstrapping test value
    ...
    protected:
        ...
        */
        ...
        void _fillAligned(const std::vector<std::string>& sat_order,
        ...
        */
        ...
        void _refreshEnvFeatures();
        void _writeFeature(const t_flt_rec& rec);
        void _writeWLIF(t_gamb_cmn* amb_cmn);
        /**
